...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.QueenAnt and insect.imposter == False:
                return
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container and not self.ant and other.container == False:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if self.min_range > 0:
            i = 0
            while i < self.min_range and nex is not None:
                nex = nex.entrance
                i = i + 1
        limit = 0
        while nex.entrance is not None and limit < self.max_range:
            if len(nex.bees) > 0:
                return random_or_none(nex.bees)
            nex = nex.entrance
            limit = limit + 1
        return None
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.watersafe == False:
            amount_armor = insect.armor
            insect.reduce_armor(amount_armor)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4A
        new_list = self.place.bees[:]
        self.armor = self.armor - amount
        if self.armor <= 0:
            for elem in new_list:
                elem.reduce_armor(self.damage)
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        new_list_two = self.place.bees[:]
        for elem in new_list_two:
            elem.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        # -check for imposter
        # -counter is 0 or 1
        # -of counter is 1, and then you create another queen ant, will be imposter
    ...
    def action(self, colony):
        ...
        while n is not None:
            ...
            if n.ant is not None and n.ant.doubling == False:
                n.ant.damage = n.ant.damage * 2 
                n.ant.doubling = True

            if n.ant is not None and n.ant.container == True and n.ant.ant is not None and not n.ant.ant.doubling:
                n.ant.ant.damage = n.ant.ant.damage * 2
                n.ant.ant.doubling = True
            n = n.exit
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 9
        if self.armor <= amount and self.imposter == False:
            return bees_win()
        else:
